...
class CheckoutLinkTest extends \WC_Unit_Test_Case {
	...
	public function test_endpoint_rewrite_is_deferred_during_installing_mode(): void {
		...
		$original_queue          = get_option( 'woocommerce_queue_flush_rewrite_rules' );
		...
		try {
			...
			$this->assertArrayNotHasKey( '^checkout-link$', get_option( 'rewrite_rules' ), 'The endpoint guard should remain open for the next normal request.' );
		} finally {
			wp_installing( false );
			update_option( 'rewrite_rules', $original_rules );
			update_option( 'woocommerce_queue_flush_rewrite_rules', $original_queue );
